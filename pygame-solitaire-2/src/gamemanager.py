...
class GameManager(object):
    ...
    def handleMouseInput(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1 and not self.mouse_held: #when mouse is clicked
                self.mouse_held = True
                self.gameBoard.mouseClicked()
        elif event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1 and self.mouse_held: #when mouse is released
                self.mouse_held = False
    ...
    def playGame(self):
        ...
        while running:
            if self.handleInput() == 1:
                running = False
            self.drawGame()
   #function that runs the program for when you win, hover over the reset button, or play a new game
    def drawGame(self):
    ...
    #function that loads the textures for the cards and suits
    def loadTextures(self):
        ...
        #function that loads the textures for the card numbers
