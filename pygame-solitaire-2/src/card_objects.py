...
class Card(object):
  ...
  #values/data the cards all have
    def __init__(self, textures, suit, val):
        ...
        #Positions to draw different parts of the cards at
        self.top_xy = (5, 5)
        self.bottom_xy = (5, CARD_DIM[1] - CARD_SMALL_ITEM_SIZE[1] - 5)
        self.top_spacing_x = CARD_DIM[0] - 30

    #actually adds the suits and value to the cards
...
#class that creates the deck object
class Deck(object):

    #values/data the deck has
    ...
    #runs what happens when you click the deck
    def clicked(self):
        card_count = 3
        if self.easy_mode:
            card_count = 1
    ...
    #runs the program for shuffleing the deck
    ...
    #runs the program for dealing cards when you click the deck
    def dealCards(self):
        if self.autowin:
            counter = 0
...
#creates the holders for the cards
class CardHolder(object):

   #basic data all the cardsholders have
    ...
    #function that moves a card from one holder to another
    ...
    #checks if a card can actually be placed in the new holder
    def checkSuitAndVal(self, card1, card2):
        sys.exit("The base class version of this function should not be used.")

    #what happens if the card can be placed where your trying to put it
...
class BottomRowHolder(CardHolder):
    ...
    #graddbing a card from the normal row
...
class TopRightHolder(CardHolder):
    ...
    #grabbing a card from the top right row
    def grabCard(self, mouse_pos, mouse_holder):
        if len(self.cards) <= 0:
            return
...
class TopLeftHolder(CardHolder):
    ...
    #grabbing a card flipped from the deck
    def grabCard(self, mouse_pos, mouse_holder):
        if len(self.cards) <= 0:
            return
...
#the holder that cards are stored in when you grab them with the mouse
